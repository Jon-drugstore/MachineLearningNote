...
当数据窗口滑动时，输入在变化，但中间滤波器$$w_0$$权重（即每个神经元连接数据窗口的权重）是不变的。**这个权重不变即所谓的CNN中参数（权重）共享机制。**
...
例如，对于$$96*96$$像素图像，假设已得到400个定义在$$8*8$$输入上的特征，每个特征和图像卷积会得到一个$$(96 − 8 + 1) * (96 − 8 + 1) = 7921$$维卷积特征。由于有400个特征，所以每个样例都会得到一个$$7921 * 400 = 3168400$$维卷积特征向量。学习超过3百万特征输入的分类器十分不便，且容易出现过拟合。
...
----
<p align="center">
  <img src="./Images/cnn27.png" width = "400"/>
</p>
