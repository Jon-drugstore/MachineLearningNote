...
**未知图案局部和标准X图案局部一个个比对时的计算过程，便是卷积操作。**
...
当窗口滑动时，输入在变化，但中间滤波器$$w_0$$权重（即每个神经元连接窗口的权重）不变。**这个权重不变即所谓CNN参数（权重）共享机制。**
