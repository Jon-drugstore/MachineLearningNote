...
目标是使所有训练数据的损失和尽可能小。如果将神经网络预测矩阵公式带入$$y_p$$中（因为$$z=y_p$$），可把损失写为关于参数的函数。这个函数称为损失函数（loss function）。

此时问题转化为优化参数，让损失函数值最小。一般使用**梯度下降算法**。梯度下降算法每次计算参数在当前的梯度，然后让参数向梯度反向前进一段距离，不断重复，直到梯度接近零截止。

在神经网络模型中，由于结构复杂，每次计算梯度代价很大。因此还需使用**反向传播算法**。反向传播算法不一次计算所有参数梯度，而是从后往前。先计算输出层梯度，然后第二个参数矩阵梯度，接着中间层梯度，再第一个参数矩阵梯度，最后输入层梯度。计算结束后，所要的两个参数矩阵的梯度就有了。
